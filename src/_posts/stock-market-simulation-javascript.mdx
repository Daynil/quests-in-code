...
Even though our mean represents the general historical trend of the stock, we know it is completely unrealistic for a stock to increase a static amount each time day. We need to inject some volatility that we know is characteristic of the stock market. For this we use $\sigma W_t$.
...
Now we can really appreciate the range of possibilities. However, as we begin to increase the number of projections, we become quite limited by what the human eye can discern. Monte Carlo simulations are most useful when you create a huge amount of simulations. If we just throw 100,000 projections on a graph, we won't be able to make much sense of it (not to mention the rendering inefficiencies).
...
A better way to visualize a large amount of simulations is to split them into quantiles.
